...
def clean_transactions(df: pd.DataFrame) -> pd.DataFrame:
    """
    Clean transaction data: drop null CustomerID, positive Qty/Price only,
    exclude cancellations, add TotalPrice, parse dates.
    """
...
def load_clean(data_path: Path | None = None) -> pd.DataFrame:
    """Load raw data and return cleaned transaction dataframe."""
